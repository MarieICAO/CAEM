...
class ExtractText(Runways) :
    ...
    def income(self) :
        "Provide an HashMap with the Country's wealth categories of the World Bank using a 'txt' input."
    ...
    def countryFeatures(self,Code) :
        "Provide an HashMap with the Country's population and land area using a 'txt' input."
        leg = self.data[0].split('\t')
        leg[-1] = leg[-1].split('\n')[0]
        Feature = CatTab(leg[0],leg)
        for line in self.data[1:] :
            l = line.split('\t')
            l[-1] = l[-1].split('\n')[0]
            if l[0]=='CHI' :
                ...
                code = Code.htab[l[0]][0]
            if code in Feature.htab :
                feat = Feature.htab[code]
                feat[2] = feat[2] + float(l[2])
                feat[3] = feat[3] + int(l[3])
            else :
                Feature.htab[code] = [Code.htab[l[0]][2],code,float(l[2]),int(l[3])]
        return Feature
    ...
    def infobase(self) :
        ...
        for line in self.data :
            ...
            if key not in Info.htab :
                Info.htab[key] = CatTab('Country Name',['Number of Flights In & Out','Number of Passenger In & Out','Global Number of Airports','Number of Small Airports','Number of Average Airports','Number of Big Airports','Global Runway National Index','Synthetic Runway Index','Number of National Airlines','World Bank Income Category'])
    ...
    def flights(self,Code,fill=0.8) :
        ...
        for line in self.htab.values() :
            if line[3] in Flight.htab :
                ...
                port[7]=port[7]+int(fill*int(line[12])*int(line[11]))
            else :
                code = Code.htab[line[6]]
                Flight.htab[line[3]] = [line[3],line[4],line[5],code[2],code[0],int(line[11]),0,int(fill*int(line[11])*int(line[12])),0]
            if line[7] in Flight.htab :
                port = Flight.htab[line[7]]
                port[6]=port[6]+int(line[11])
                port[8]=port[8]+int(fill*int(line[12])*int(line[11]))
            else :
                code = Code.htab[line[10]]
                Flight.htab[line[7]] = [line[7],line[8],line[9],code[2],code[0],0,int(line[11]),0,int(fill*int(line[11])*int(line[12]))]
        return Flight
...
class GraphCat :
    ...
    def printFlights(self,name) :
        ...
        x = list(self.htab.keys())
        legend = [None,'Number of Flight','Number of Passenger']
        for k in range(1,3) :
            plt.subplot(1,2,k)
            y = [line.htab[name][k-1] for line in self.htab.values() if name in line.htab]
            y[:0] = [0 for l in range(len(x)-len(y))]
            plt.bar(x,y,width=0.5,color='#5b92e5')
            axes = plt.gca()
            n = 1 
            for label in plt.gca().xaxis.get_ticklabels():
        ...
        plt.suptitle('Air Traffic',fontsize=36,x=0.5,y=0.97)
        plt.show()
    ...
    def printRunways(self,name) :
        ...
        x = list(self.htab.keys())
        legend = [None,'Global Runway National Index','Synthetic Runway Index']
        for k in range(1,3) :
            plt.subplot(1,2,k)
            y = [line.htab[name][k+5] for line in self.htab.values() if name in line.htab]
            y[:0] = [0 for l in range(len(x)-len(y))]
            plt.bar(x,y,width=0.5,color='#5b92e5')
            axes = plt.gca()
            n = 1 
    ...
    def printAirlines(self,name) :
        ...
        x = list(self.htab.keys())
        y = [line.htab[name][8] for line in self.htab.values() if name in line.htab]
    ...
    def printIncome(self,name) :
        ...
        y = [Switcher[line.htab[name][9]] for line in self.htab.values() if name in line.htab]
        y[:0] = [0 for k in range(len(x)-len(y))]
    ...
    def printCountry(self,Results,name,sameaxis=True) :
        ...
        limit = str(max([line.htab[name][2] for line in self.htab.values() if name in line.htab]))
...
class CatTab(Runways,OAGTab,GraphCat) :
    ...
    def airport(self,Flights) :
        ...
        Airports = CatTab('IATA Airport Code',['IATA Airport Code','ICAO Airport Code','Airport Name','City Name','Country Code','Country Name','Number of Runways','Runway Index','Number of Flights In & Out','Number of Passenger In & Out','Number of Airlines'])
        for key,line in Flights.htab.items() :
            if key in self.htab :
                port = self.htab[key]
                Airports.htab[port[0]] = port.copy()
                Airports.htab[port[0]].extend([max(line[5:7]),max(line[7:9]),line[9]])
            else :
    ...
    def classement(self,sizes) :
        "Add the category (based on the attendance levels) of the each Airport in the Airports' database. The argument 'sizes' gives the percentage of airports in each categories in ascending order."
        if sum(sizes)!=1.0 :
    ...
    def major(self,lim,prop) :
        ...
        for key,line in self.htab.items() :
            if line[4] in PortbyState.htab :
                PortbyState.htab[line[4]].append([line[0],line[8],line[11]])
            else :
                PortbyState.htab[line[4]] = [[line[0],line[8],line[11]]]
        Major = CatTab('IATA Airport Code',['IATA Airport Code','ICAO Airport Code','Airport Name','City Name','Country Code','Country Name','Number of Runways','Runway Index','Number of Flights In & Out','Number of Passenger In & Out','Number of Airlines','Category of the Airport'])
    ...
    def land2(self,nb):
        ...
        World = CatTab('Country Code',['Country Code','Country Name','Number of Flights In & Out','Number of Passenger In & Out','Global Number of Airports','Number of Airports by Categories of Size'])
        for key,line in Country.htab.items() :
            ...
            for port in line :
                Fl = Fl+port[8]
                Pas = Pas+port[9]
                Category[port[11]-1] = Category[port[11]-1]+1
            World.htab[key].extend([Fl,Pas,sum(Category)])
            World.htab[key].extend(Category)
        return World
    ...
    def airportStats(self,sizes,year) :
        ...
        for line in self.htab.values() :
            if line[8]>limit[line[11]] :
                limit[line[11]] = line[8]
        return limit
    ...
    def classification(self,Code,Runways,Income,year,sizes=(0.42,0.42,0.16),lim=3,prop=0.5) :
        ...
        Info = CatTab('Country Code',['Country Code','Country Name','Number of Flights In & Out','Number of Passenger In & Out','Global Number of Airports','Number of Small Airports','Number of Average Airports','Number of Big Airports','Global Runway National Index','Synthetic Runway Index','Number of National Airlines','World Bank Income Category'])
    ...
    def connectivity_index(self,Feature) :
        "Provide an HashMap with several indicators to determine the Civil Aviation Connectivity of a Country using the yearly Infos' database."
        Connectivity = CatTab('Field','List of Index in this Field')
        Connectivity.htab['Trafic'] = CatTab('Country Code',['Country Code','Country Name','Number of Flights','Number of Passenger','Average Number of Flight by Inhabitant'])
        Connectivity.htab['Global Capacity'] = CatTab('Country Code',['Country Code','Country Name','Global Number of Airports','Average Distance to an Airport (km)','Number of Inhabitants by Airport'])
        Connectivity.htab['Facility']  = CatTab('CountryCode',['Country Code','Country Name','Global Runway Index','Synthetic Runway Index','Number of Airlines'])
        for key,line in self.htab.items() :
            Trafic = Connectivity.htab['Trafic'].htab
            Trafic[key] = Feature.htab[key][0:2]
            Trafic[key].extend(line[0:2])
            Trafic[key].append(int(line[1]/Feature.htab[key][3]))
            GCapacity = Connectivity.htab['Global Capacity'].htab
            GCapacity[key] = Feature.htab[key][0:2]
            GCapacity[key].extend([line[2],sqrt(Feature.htab[key][2]/(pi*line[2])),int(Feature.htab[key][3]/line[2])])
            Facility = Connectivity.htab['Facility'].htab
            Facility[key] = Feature.htab[key][0:2]
            Facility[key].extend(line[6:9])
        return Connectivity